module InlineSvg exposing (inlineSvg, makeSvg)
...
svgNamespace =
    property "namespace" (Json.Encode.string "http://www.w3.org/2000/svg")


svg : List (Attribute msg) -> List (Html msg) -> Html msg
svg attributes children =
    node "svg" (svgNamespace :: attributes) children


innerHtml : String -> Attribute msg
innerHtml value =
    property "innerHTML" (Json.Encode.string value)


makeSvg : SvgData -> List (Attribute msg) -> Html msg
makeSvg svgData customAttributes =
    let
        attributes =
            svgData.attrs
                |> List.map (\{ name, value } -> attribute name value)
    in
        svg (innerHtml svgData.content :: (attributes ++ customAttributes)) []


inlineSvg : String -> List (Attribute msg) -> Html msg
inlineSvg assetId attributes =
    let
        maybeAsset =
            assets
    ...
    in
        case maybeAsset of
            Just asset ->
                makeSvg asset.svgData attributes
...
type alias SvgData =
    { attrs : List { name : String, value : String }
    , content : String
    }
...
assets =
    [ { id = "arrow-down"
      , svgData =
            { content = "<title>arrow-down</title><path d=\"M2 5.772l5.205 5.756.427.472.427-.472 5.155-5.698-.854-.772-4.728 5.254L2.853 5z\"/>"
            , attrs =
                [ { name = "width", value = "16" }
    ...
    , { id = "search-big"
      , svgData =
            { content = "<path d=\"M22.063 23.696c-.453 0-.878-.176-1.199-.496l-6.416-6.416.707-.707 6.416 6.416c.132.131.306.203.491.203h.001a.695.695 0 0 0 .49-1.186l-6.415-6.416.707-.707 6.416 6.416a1.697 1.697 0 0 1-1.198 2.893zM9.535 18.7C4.448 18.7.309 14.56.309 9.473S4.448.248 9.535.248c5.086 0 9.225 4.139 9.225 9.226s-4.139 9.225-9.225 9.225zm0-17.452c-4.536 0-8.226 3.69-8.226 8.226 0 4.535 3.69 8.225 8.226 8.225s8.225-3.69 8.225-8.225c0-4.536-3.69-8.226-8.225-8.226z\"/><path d=\"M3.602 10.468a.5.5 0 0 1-.499-.483A6.73 6.73 0 0 1 5.085 4.95a6.706 6.706 0 0 1 5.034-1.982.5.5 0 0 1 .483.516c-.009.277-.239.522-.516.483a5.724 5.724 0 0 0-4.294 1.69 5.742 5.742 0 0 0-1.69 4.295.5.5 0 0 1-.483.516h-.017z\"/><path fill=\"none\" d=\"M0 0h24v24H0z\"/>"
            , attrs =
                [ { name = "width", value = "24" }
